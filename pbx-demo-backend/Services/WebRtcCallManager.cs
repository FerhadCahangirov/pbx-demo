...
public sealed class WebRtcCallManager
{
    ...
    public async Task<BrowserCallView> PlaceCallAsync(string callerSessionId, string destinationExtension, CancellationToken cancellationToken)
    {
        ...
        await PersistWebRtcCallAsync(call, "browser.call.created", cancellationToken);
    ...
    public async Task AnswerCallAsync(string sessionId, string callId, CancellationToken cancellationToken)
    {
        ...
        {
            await PublishCallUpdateAsync(call);
            await PersistWebRtcCallAsync(call, "browser.call.answered", cancellationToken);
        }
    }
    ...
    public async Task RejectCallAsync(string sessionId, string callId, CancellationToken cancellationToken)
    {
        ...
        if (completedCall is not null)
        {
            ...
            await PersistWebRtcCallAsync(completedCall, "browser.call.rejected", cancellationToken);
        }
    }
    ...
    public async Task EndCallAsync(string sessionId, string callId, CancellationToken cancellationToken)
    {
        ...
        {
            await PublishCallUpdateAsync(completedCall);
            await PersistWebRtcCallAsync(completedCall, "browser.call.ended", cancellationToken);
        }
    }
    ...
    public async Task MarkCallConnectedAsync(string sessionId, string callId, CancellationToken cancellationToken)
    {
        ...
        {
            await PublishCallUpdateAsync(call);
            await PersistWebRtcCallAsync(call, "browser.call.connected", cancellationToken);
        }
    }
    ...
    public async Task ForwardWebRtcSignalAsync(string sessionId, WebRtcSignalRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            await PublishCallUpdateAsync(call);
            await PersistWebRtcCallAsync(call, "browser.call.signal-answer", cancellationToken);
        }
    ...
    public async Task HandleSessionDisconnectedAsync(string sessionId)
    {
        ...
        foreach (var call in endedCalls)
        {
            ...
            {
                await PublishCallUpdateAsync(call);
                await PersistWebRtcCallAsync(call, "browser.call.peer-disconnected", CancellationToken.None);
            }
            catch (Exception ex)
            {
    ...
    private async Task PersistWebRtcCallAsync(BrowserCallRecord call, string eventType, CancellationToken cancellationToken)
    {
        try
        {
            var now = DateTimeOffset.UtcNow;

            var callerUpdate = new WebRtcCallCdrUpdate
            {
                CallId = call.CallId,
                OperatorUserId = call.CallerUserId,
                OperatorUsername = call.CallerUsername,
                OperatorExtension = call.CallerExtension,
                Direction = SoftphoneCallDirection.Outgoing,
                Status = call.Status,
                RemoteParty = call.CalleeExtension,
                RemoteName = call.CalleeUsername,
                StartedAtUtc = call.CreatedAtUtc,
                AnsweredAtUtc = call.AnsweredAtUtc,
                EndedAtUtc = call.EndedAtUtc,
                IsEnded = string.Equals(call.Status, StatusEnded, StringComparison.Ordinal),
                EndReason = call.EndReason,
                OccurredAtUtc = now,
                EventType = eventType
            };

            var calleeUpdate = new WebRtcCallCdrUpdate
            {
                CallId = call.CallId,
                OperatorUserId = call.CalleeUserId,
                OperatorUsername = call.CalleeUsername,
                OperatorExtension = call.CalleeExtension,
                Direction = SoftphoneCallDirection.Incoming,
                Status = call.Status,
                RemoteParty = call.CallerExtension,
                RemoteName = call.CallerUsername,
                StartedAtUtc = call.CreatedAtUtc,
                AnsweredAtUtc = call.AnsweredAtUtc,
                EndedAtUtc = call.EndedAtUtc,
                IsEnded = string.Equals(call.Status, StatusEnded, StringComparison.Ordinal),
            ...
            await _callCdrService.UpsertWebRtcCallAsync(callerUpdate, cancellationToken);
            await _callCdrService.UpsertWebRtcCallAsync(calleeUpdate, cancellationToken);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "Failed to persist browser call CDR update. CallId={CallId}, EventType={EventType}",
                call.CallId,
                eventType);
        }
    }
    ...
    private sealed class BrowserCallRecord
    {
        ...
        public required int CalleeUserId { get; init; }
