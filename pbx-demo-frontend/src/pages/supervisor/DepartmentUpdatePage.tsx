...
export function DepartmentUpdatePage({
  ...
  onReset,
  onGoRead
}: DepartmentUpdatePageProps) {
  if (selectedDepartmentId === null) {
    return (
      <section className="supervisor-page-stack">
        <article className="card">
          <h3>Departments / Update</h3>
          <p className="history-summary">Select a department from the read page first.</p>
          <button className="secondary-button" type="button" onClick={onGoRead}>
            Go to Departments / Read
          </button>
        </article>
      </section>
    );
  ...
  return (
    <section className="supervisor-page-stack">
      <article className="card">
        <h3>Departments / Update</h3>
        <p className="history-summary">Editing department #{selectedDepartmentId}</p>
      </article>
